"""
app.py — Non-RAPDRP NLP-to-SQL FastAPI backend
==============================================
...
This is the Non-RAPDRP twin of the RAPDRP backend. Only schema_metadata.py,
...
# .env lives in the Non-RAPDRP folder (one level up from backend/).
...
def get_llm():
    ...
    if not GEMMA_API_KEY:
        raise RuntimeError(
            "GEMMA_API_KEY is not set. Add it to Non-RAPDRP/.env to enable NLP-to-SQL."
        )
...
def validate_sql(sql: str):
    ...
    # Must reference our table. The label has a space, so match the distinctive
    # word "rapdrp" case-insensitively (covers "Non RAPDRP").
    if "rapdrp" not in s.lower():
...
def _fallback_answer(question: str, cols, rows, kind: str) -> str:
    ...
    if kind == "empty":
        return ("No records matched that query. The filter may be too narrow, or "
                "the value isn't present (division/circle names are stored UPPERCASE).")
    if kind == "scalar":
...
def _suggest_follow_up(question, cols, rows, kind: str, asked) -> str:
    ...
    try:
        fu = llm_generate(
            "You suggest the single most useful NEXT question a BESCOM Non-RAPDRP "
            ...
            "DOMAIN: Non-RAPDRP is a single-period DCB snapshot. The unit of business "
            "is the electrical INSTALLATION (one metered connection). There is NO "
            ...
            "targets, forecasts, causes, AT&C losses, transformers, GST/write-offs "
            "(all zero here), or any concept not represented by a column below.\n"
        ...
        with db_cursor() as cur:
            cur.execute(f"""
                SELECT
                    ...
                    SUM(total_installations_10_11)                    AS total_installations,
                    SUM(active_installations)                         AS active_installations,
                    SUM(metered_installations)                        AS metered_installations,
                    ROUND(SUM(net_consumption_33_34)::numeric / 1e6, 1) AS net_consumption_mu,
    ...
    if not GEMMA_API_KEY:
        return JSONResponse(status_code=503, content={
            ...
            "answer": "The Gemma API key isn't configured yet. Add GEMMA_API_KEY "
                      "to Non-RAPDRP/.env and restart the server.",
        })
